...
- **Stale vision flags** on opencode-go Kimi K3 / Qwen max (they take images; we treated them as text-only and always spent a bridge call). Gone: no hardcoded flags — image support resolves dynamically from models.dev/OpenRouter, pins via `models vision <p/m> on|off|auto`, and a native send rejected with 400 retries once through the bridge.
...
- **Catalog-only providers showed an empty picker.** `models refresh` GETs `/models` and stores new ids as extras. Setup now auto-runs it after keys, so the registry keeps only wire-protocol exceptions (`protocol: messages`, `upstream` renames) instead of seed lists that rot.
